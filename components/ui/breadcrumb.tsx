                ...
                {items.map((item, index) => (
                    <li key={index} aria-current={item.active ? 'page' : undefined} className='inline-flex items-center'>
                        ...
                        {item.active ? (
                            ...
                            <a href={item.href} className={`inline-flex items-center text-sm font-medium text-gray-700 hover:text-blue-600 dark:text-gray-400 dark:hover:text-white ${index === 0 ? '' : 'ms-1 md:ms-2'}`}>
                                {index === 0 && (
